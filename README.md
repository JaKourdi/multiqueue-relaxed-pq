...
1. **Problem: skew breaks the bound (~5×).** Under a non-uniform *placement* workload (small keys routed to a few "hot" queues), the measured mean rank error is about 5× the uniform-load prediction: **27.56 vs 5.69** at `n=8`, `d=2` (4.85×). Changing only key *order* while keeping placement uniform (a Dijkstra-like MONOTONE workload) is harmless (2.25). Rank error is ordinal, so what the guarantee depends on is non-uniform placement, not key randomness.
...
|------|-------------|--------|
| **1 — problem** | mean rank error, `n=8`, `d=2` (theory = `(5/6)·8 − 1 + 1/(6·8) = 5.69`) | UNIFORM **5.62** (0.99×) · MONOTONE **2.25** (0.40×) · SKEWED **27.56** (4.85×) |
